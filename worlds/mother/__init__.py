...
from .Options import MOTHEROptions
from .Regions import create_regions
from .Rules import set_rules
from .Types import ChapterType, chapter_type_to_name
from .Rom import generate_output, MOTHERProcedurePatch

from .Client import MOTHERClient

import settings

class MOTHERSettings(settings.Group):
    class MOTHERRomFile(settings.UserFilePath):
        """File name of the MOTHER ROM file"""
        description = "EARTH BOUND 1.0 ROM file"
        copy_to = "mother.nes"
        md5s = [MOTHERProcedurePatch.hash]

    rom_file: MOTHERRomFile = MOTHERRomFile(MOTHERRomFile.copy_to)
...
class MOTHERWeb(WebWorld):
    ...
    tutorials = [Tutorial(
        ...
        "A guide to setting up MOTHER for Archipelago. "
...
class MOTHERWorld(World):
    ...
    options_dataclass = MOTHEROptions
