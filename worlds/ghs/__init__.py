...
from .Options import GHSOptions
from .Regions import create_regions
from .Types import GHSItem, EpisodeType, episode_type_to_name, episode_type_to_shortened_name
...
def run_client():
    from .GHSClient import launch_client
    launch_subprocess(launch_client, name="GHSClient")

icon_paths["ghs_ico"] = f"ap:{__name__}/icon.png"
components.append(
    Component("Gregory Horror Show Client", func=run_client, component_type=Type.CLIENT, icon="ghs_ico")
)

class GHSWeb(WebWorld):
    theme = "ocean"
    tutorials = [Tutorial(
        "Multiworld Setup Guide",
        "A guide to setting up Gregory Horror Show for Archipelago. "
...
class GHSWorld(World):
    """
    Gregory Horror Show is scary :(
    """

    game = "Gregory Horror Show"
    item_name_to_id = {name: data.ap_code for name, data in item_table.items()}
    location_name_to_id = get_location_names()
    options_dataclass = GHSOptions
    options = GHSOptions
    web = GHSWeb()
    ...
    def create_items(self):
        ...
        for event, item in event_item_pairs.items():
            event_item = GHSItem(item, ItemClassification.progression_skip_balancing, None, self.player)
