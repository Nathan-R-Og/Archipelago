...
from .Types import ItemData, GHSItem, EpisodeType, episode_type_to_name, episode_type_to_shortened_name
...
def create_itempool(world: "GHSWorld") -> List[Item]:
    itempool: List[Item] = []
...
def create_item(world: "GHSWorld", name: str) -> Item:
    data = item_table[name]
    return GHSItem(name, data.classification, data.ap_code, world.player)


def create_multiple_items(world: "GHSWorld", name: str, count: int = 1,
                          item_type: ItemClassification = ItemClassification.progression) -> List[Item]:
    data = item_table[name]
    itemlist: List[Item] = []

    for i in range(count):
        itemlist += [GHSItem(name, item_type, data.ap_code, world.player)]

    return itemlist


def create_junk_items(world: "GHSWorld", count: int) -> List[Item]:
