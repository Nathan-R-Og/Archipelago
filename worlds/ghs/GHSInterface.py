...
class GHSInterface(GameInterface):
    moves_locked = False
    def get_current_episode(self) -> GHSEpisode:
        episode_num = self._read32(self.addresses["world id"])
        return GHSEpisode(episode_num)
    ...
    def skip_cutscene(self, ctx: 'GHSContext') -> None:
    ...
    def get_current_level_name(self) -> str:
        level_addresses = ADDRESSES["SLES-51933"]["levels"]
        current_address = self.get_current_address()
        current_episode = self.get_current_episode()

        if current_episode in (GHSEpisode.Paris, GHSEpisode.Cold_Heart_of_Hate) or current_address == 8:
            return "N/A"
    ...
    def write_names(self, ctx: 'GHSContext') -> None:
