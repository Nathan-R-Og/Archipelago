...
class GHSContext(CommonContext):
    command_processor = GHSCommandProcessor
    game_interface: GHSInterface
    ...
    #Game state
    current_episode: Optional[GHSEpisode] = None
    thief_moves: int = 0
    bosses_beaten: int = 0
    ...
    def __init__(self, server_address, password):
        ...
        self.game_interface = GHSInterface(logger)

    async def server_auth(self, password_requested: bool = False) -> None:
        if password_requested and not self.password:
            await super(GHSContext, self).server_auth(password_requested)
    ...
    def on_package(self, cmd: str, args: dict):
        if cmd == "Connected":
            ...
            self.current_scene_key = f"GHS_current_scene_T{self.team}_{self.slot}"

def update_connection_status(ctx: GHSContext, status: bool):
...
async def pcsx2_sync_task(ctx: GHSContext):
...
async def _handle_game_ready(ctx: GHSContext) -> None:
...
def launch_client():
    ...
    async def main():
        ...
        ctx = GHSContext(args.connect, args.password)
