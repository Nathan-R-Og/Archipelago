...
from .GHSInterface import GHSEpisode, GHSInterface
...
SAVE_FILE = "GHS_item_progress.json"

if TYPE_CHECKING:
    from .GHSClient import GHSContext

async def update(ctx: 'GHSContext', ap_connected: bool) -> None:
    """Called continuously"""
...
async def init(ctx: 'GHSContext', ap_connected: bool) -> None:
    """Called when the player connects to the AP server"""
    if ap_connected:
        ctx.in_game = True

def check_levels(ctx: 'GHSContext') -> None:
...
def check_maps(ctx: 'GHSContext') -> None:
...
def check_keys(ctx: 'GHSContext') -> None:
    ...
    if (moves & all_moves) == all_moves:
        moves &= ~MOVES["Hacking"]
    if (ctx.game_interface._read32(move_address) != moves) and (GHSInterface.moves_locked is False):
        ctx.game_interface._write32(move_address, moves)

def check_hubs(ctx: 'GHSContext') -> None:
...
def check_bottles(ctx: 'GHSContext') -> None:
    if ctx.slot_data is None:
        return
...
def check_bosses(ctx: 'GHSContext') -> None:
    if ctx.slot_data is None:
        return
    options = ctx.slot_data.get("options", {})
...
async def handle_checks(ctx: 'GHSContext') -> None:
...
async def handle_goal(ctx: 'GHSContext') -> None:
    if 10020233 in ctx.locations_checked:
        await ctx.send_msgs([{"cmd": "StatusUpdate", "status": ClientStatus.CLIENT_GOAL}])

async def handle_received(ctx: 'GHSContext') -> None:
    ...
    for i in range(start_index, len(network_items)):
        ...
        if (10020001 <= item.ap_code <= 10020014) and (GHSInterface.moves_locked is False):
...
def get_blueprint(episode: int) -> Optional[str]:
    blueprint_mapping = {
        GHSEpisode.Tide_of_Terror: "ToT Blueprints",
        GHSEpisode.Sunset_Snake_Eyes: "SSE Blueprints",
        GHSEpisode.Vicious_Voodoo: "VV Blueprints",
        GHSEpisode.Fire_in_the_Sky: "FitS Blueprints",
    }
    return blueprint_mapping.get(GHSEpisode(episode))
